...
class Mygenerator_multitasking(Sequence):
    ...
    def __getitem__(self, idx):
        batch_x = self.x[idx * self.batch_size:(idx + 1) * self.batch_size]
        batch_y_seg = self.y[idx * self.batch_size:(idx + 1) * self.batch_size]
        batch_y_bound = get_boundary_labels(self.y)[idx * self.batch_size:(idx + 1) * self.batch_size]
        batch_y_dist = get_distance_labels(self.y)[idx * self.batch_size:(idx + 1) * self.batch_size]
        batch_y_color = get_color_labels(self.x)[idx * self.batch_size:(idx + 1) * self.batch_size]
